...
4. **The FSKU Fix** — one published reading per GPU family, free:
   ...
   - **Two readings, both published.** *Listed* is the headline above: what the on-demand/spot ask is. *Standardized* answers what a quote-based benchmark that standardizes rental term answers: **one quote per seller — its lowest per-GPU price for the family on any term (on-demand, spot/preemptible, reserved) — median across sellers**, neocloud tiers, at least 3 sellers, no winsorizing and no fitted parameters. Measured 2026‑09‑20 on a 62-row tape it sat within ~10% of Silicon Data's H100/H200/B200/A100 readings; the listed reading sat ~35–50% above them, which is the gap between the ask on a price page and the term a buyer actually books. Both go into the daily history.
